...
class Kinect():
    ...
    def get_depthframe(self):
        '''
        只获得深度数据，实时，单位m
        '''
        # Get undistorted (and registered) frames
        frames = self.listener.waitForNewFrame()
        depth_frame = self._get_depth_frame(frames)
        self.listener.release(frames)

        return depth_frame

    def get_irframe(self):
        '''
        只获得红外数据，实时
        '''
        # Get undistorted (and registered) frames
        frames = self.listener.waitForNewFrame()
        ir_frame = self._get_ir_frame(frames)
        self.listener.release(frames)
    ...
    def get_depth_ptcld(self, depth_data, scale=1000):
        '''
            只对深度数据转换为真实三维数据
        '''
        # Get point cloud 
        ptcld = self._depthMatrixToPointCloudPos(depth_data, self._camera_params, scale=scale)
